...
import {backend_util, Conv2D, Conv2DAttrs, Conv2DInputs, KernelConfig, TypedArray, util} from '@tensorflow/tfjs-core';
...
export const conv2DConfig: KernelConfig = {
  ...
  kernelFunc: ({inputs, backend, attrs}) => {
    const {x, filter} = inputs as Conv2DInputs;
    ...
    const yBatchStride = yStrides[0];
    const yRowStride = isChannelsLast ? yStrides[1] : yStrides[2];
    const yColStride = isChannelsLast ? yStrides[2] : 1;
    const yChannelStride = isChannelsLast ? 1 : yStrides[1];
