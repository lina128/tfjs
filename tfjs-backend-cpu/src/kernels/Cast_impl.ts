...
import {DataType, KernelBackend, scalar, Tensor, TypedArray, util, zeros} from '@tensorflow/tfjs-core';
import {identityConfig} from './Identity';

export function castTensor<T extends Tensor>(
    xVals: TypedArray, dtype: DataType, backend: KernelBackend,
    xDType: DataType): T {
  if (dtype === 'complex64') {
    if (xDType === 'complex64') {
      const identityKernel = identityConfig.kernelFunc;
      return;
    }
